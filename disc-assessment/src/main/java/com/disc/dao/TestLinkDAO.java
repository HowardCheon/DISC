...
public class TestLinkDAO {
    ...
    /**
     * Get recent test links with user information for admin history
     */
    public List<Map<String, Object>> getRecentTestLinksWithUserInfo(int limit) throws SQLException {
        String sql = """
            SELECT tl.*, u.name as user_name, u.created_at as user_created_at
            FROM test_links tl
            JOIN users u ON tl.user_id = u.id
            ORDER BY tl.created_at DESC
            LIMIT ?
    ...
    /**
     * Get test links by user name with pagination
     */
    public List<Map<String, Object>> getTestLinksByUserName(String userName, int offset, int limit) throws SQLException {
        String sql = """
    ...
    /**
     * Get count of test links by user ID
     */
    public int getTestLinkCountByUserId(Long userId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM test_links WHERE user_id = ?";
